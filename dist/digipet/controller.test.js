...
describe("walkDigipet", () => {
    it("increases digipet happiness by 10 and decreases nutrition by 5 (to represent need for sustenance)", () => {
        // setup
        model_1.setDigipet(model_1.INITIAL_DIGIPET); //updating Steve 
        expect(model_1.getDigipet()).toStrictEqual(model_1.INITIAL_DIGIPET); //providing the current version of Steve
        // act
    ...
    it("cannot increase happiness past 100", () => {
        // setup
        model_1.setDigipet({ happiness: 96, nutrition: 50, discipline: 50 });
        // act
        controller_1.walkDigipet();
        // assert
...
describe("ignoreDigipet", () => {
    test("decrease all stats by 10 (if all digipet stats are above 10)", () => {
        model_1.setDigipet(model_1.INITIAL_DIGIPET);
        expect(model_1.getDigipet()).toStrictEqual(model_1.INITIAL_DIGIPET); //test really to check whether setDigipet and getDigipet work....
        controller_1.ignoreDigipet();
        expect(model_1.getDigipet()).toStrictEqual({ 'happiness': model_1.INITIAL_DIGIPET.happiness - 10, 'nutrition': model_1.INITIAL_DIGIPET.nutrition - 10, 'discipline': model_1.INITIAL_DIGIPET.discipline - 10 });
    });
    it("cannot decrease stats below 0", () => {
        // setup
        model_1.setDigipet({ happiness: 9, nutrition: 5, discipline: 0 });
        // act
        controller_1.ignoreDigipet();
        // assert
        // expect(getDigipet()).toHaveProperty("nutrition", 0);
        expect(model_1.getDigipet()).toStrictEqual({ 'happiness': 0, 'nutrition': 0, 'discipline': 0 });
    });
});
